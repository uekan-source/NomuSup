class AlcoholSimulationService
  def initialize(weight:, gender:, constitution:, drinks:)
    ...
    @weight = weight.to_f > 0 ? weight.to_f : 60.0
    @gender = gender
  ...
  def execute
    ...
    # 分解時間を計算（ゼロ除算回避）
    time_in_hours = metabolism_rate > 0 ? pure_alcohol / metabolism_rate : 0
  ...
  def generate_advice(alcohol_g, hours)
    ...
    # 純アルコール量に応じた厚労省基準の警告
    if alcohol_g >= 40
      advices << "⚠️ 【多量飲酒のサイン】\n純アルコール量が40gを超えています。厚生労働省が推奨する「節度ある適度な飲酒」の基準を大きく上回っており、肝機能への負担が懸念されます。脱水を防ぐため、同量以上の水分（和らぎ水）を必ず摂取してください。"
    elsif alcohol_g >= 20
      advices << "💡 【適量オーバーの目安】\n純アルコール量が20gを超えています。これ以上のペースで飲むと、翌朝にアルコールが残りやすくなります。チェイサーを挟みながらゆっくり楽しみましょう。"
    end

    # 分解時間に応じた運転等の警告（免責事項の強調）
    if hours > 0
      advices << "🚗 【重要：運転について】\nアルコールが完全に抜けるまで【#{format_time(hours)}】ほどかかる見込みです。※この時間はあくまで計算上の目安であり、当日の体調や空腹状態によってさらに長引く可能性があります。完全に抜けるまでは、車の運転や危険な作業は絶対にお控えください。"
    else
